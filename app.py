...
# Signed OAuth state carries the PKCE verifier so the callback can
# complete even if the Render instance restarts between redirects.
...
def verify_oauth_state(state: str) -> tuple[bool, str | None]:
    try:
        ...
        if issued_at > now or now - issued_at > OAUTH_STATE_MAX_AGE:
            return False, None
...
def oauth_callback(
    ...
    valid_state, code_verifier = verify_oauth_state(state)

    if not valid_state or not code_verifier:
        raise HTTPException(
            status_code=400,
...
# Compatibility alias. Canonical callback is /oauth/callback.
...
def youtube_connect():
    return RedirectResponse("/oauth/start", status_code=302)
