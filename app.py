...
def upload_queue_run(request: Request):
    ...
    try:
        ...
        # Upload in fixed batches of UPLOAD_DAILY_LIMIT items.
        # The first batch starts immediately. Once a full batch has
        # finished, the worker waits 24 hours from the LAST upload in
        # that batch before starting the next batch.
        uploaded_items = [
            item for item in data["items"]
            if item.get("status") == "uploaded"
            and _queue_timestamp(item.get("uploaded_at")) > 0
        ]
        uploaded_total = len(uploaded_items)
        batch_position = uploaded_total % UPLOAD_DAILY_LIMIT

        if uploaded_total > 0 and batch_position == 0:
            last_uploaded_at = max(
                _queue_timestamp(item.get("uploaded_at"))
                for item in uploaded_items
            )
            wait_remaining = 86400 - (now - last_uploaded_at)
            if wait_remaining > 0:
                return {
                    "ok": True,
                    "started": False,
                    "reason": "waiting 24 hours after completed upload batch",
                    "wait_seconds": int(wait_remaining),
                    "status": _queue_status(data),
                }

        # Recover jobs left in `processing` by a hard crash/restart.
        # Normal upload failures already return to `queued` below, so this
        # only handles the case where Python/Render dies before the handler
        # can clean the row up.
        ...
        for stale_item in (stale_result.data or []):
            ...
            db.table("upload_queue").update({
                "status": "queued",
                "error": (
                    f"Recovered stale processing job after {UPLOAD_PROCESSING_TIMEOUT}s; retrying."
                ),
                "retry_count": (stale_item.get("retry_count") or 0) + 1,
            }).eq("video_id", stale_video_id).eq("status", "processing").execute()

        # Re-read the queue after recovery so the response/status and the
        # next-item selection reflect the repaired state.
        data = _load_queue()

        # Queue age does NOT control eligibility. A newly scanned Short
        # can be uploaded immediately if the current batch has capacity.
        result = (
            db.table("upload_queue")
            .select("*")
            .eq("status", "queued")
        ...
        try:
            ...
            db.table("upload_queue").update({
                "status": "queued",
                "error": ("YouTube daily upload limit reached; waiting for reset." if daily_limit else error_text[-2000:]),
                "retry_count": (item.get("retry_count") or 0) + 1,
            }).eq("video_id", video_id).execute()
            if daily_limit:
                return {"ok": True, "started": True, "waiting_for_daily_limit": True}
            raise HTTPException(status_code=502, detail=f"Queued upload failed for {video_id}")
