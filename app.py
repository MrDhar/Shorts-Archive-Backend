...
def upload_queue_run(request: Request):
    ...
    try:
        ...
        uploaded_24h = sum(
            ...
            and now - _queue_timestamp(item.get("uploaded_at")) < 86400
        )
        ...
        # A queued Short must remain in the queue for at least 24 hours
        # before it becomes eligible for upload. The existing created_at
        # timestamp in Supabase is used as the queue-entry time, so no
        # schema change is required and this survives Render spin-downs.
        eligible_cutoff = time.strftime(
            "%Y-%m-%dT%H:%M:%SZ",
        ...
        result = (
            ...
            .select("*")
            .eq("status", "queued")
            .lte("created_at", eligible_cutoff)
            .order("created_at", desc=False)
            .limit(1)
            .execute()
        )
