...
# Each state also stores its PKCE code_verifier so the callback
# can complete the same OAuth transaction.
OAUTH_STATES = {}
...
def oauth_start():
    ...
    # Explicitly enable PKCE and retain the verifier until Google
    # returns to the callback. A new Flow object is created in the
    # callback, so the verifier must be carried across the request.
    ...
    if not flow.code_verifier:
        raise HTTPException(
            status_code=500,
            detail="Could not create OAuth PKCE verifier.",
        )
...
def oauth2callback(code: str | None = None, state: str | None = None, error: str | None = None):
    ...
    # Recover the exact PKCE verifier created for this authorization.
    ...
    try:
        ...
        # Supply the same PKCE verifier used in /oauth/start.
