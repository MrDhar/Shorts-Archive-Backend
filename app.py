...
def upload_queue_run(request: Request):
    ...
    try:
        ...
        # A batch is UPLOAD_DAILY_LIMIT successful uploads. The 24-hour
        # cooldown starts at the LAST successful upload in the batch.
        uploaded_items = [
        ...
        data = _load_queue()
        target_successes = UPLOAD_DAILY_LIMIT - batch_position

        # Snapshot queued items for this invocation. A failed Short is
        # requeued but is not retried in a tight loop during this request.
        queued_items = [
            item for item in data["items"]
        ...
        for item in queued_items:
            if batch_successes >= target_successes:
                break
            ...
            try:
                ...
                db.table("upload_queue").update({
                    "status": "queued",
                    "error": (
                        "YouTube daily upload limit reached; waiting for reset."
                        if daily_limit else error_text[-2000:]
                    ),
                ...
                # Try another queued Short. Retry this failed one later.
                continue

        final_data = _load_queue()
        completed = batch_successes == target_successes

        logger.info(
            "Queue batch finished: uploaded=%d target=%d attempts=%d completed=%s",
            batch_successes,
            target_successes,
            batch_attempts,
            completed,
        ...
        return {
            "ok": True,
            "started": batch_successes > 0,
            "uploaded_this_batch": batch_successes,
            "target_this_batch": target_successes,
