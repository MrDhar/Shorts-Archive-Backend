...
DISCOVER_BATCH_SIZE = int(os.getenv("DISCOVER_BATCH_SIZE", "500"))
# Safety ceiling per request/session. The frontend can page through batches.
MAX_DISCOVER = int(os.getenv("MAX_DISCOVER", "50000"))

# Persistent metadata-only upload queue. MP4 files are never stored here.
UPLOAD_QUEUE_FILE = Path(os.getenv("UPLOAD_QUEUE_FILE", "/data/upload_queue.json"))
UPLOAD_DAILY_LIMIT = max(1, int(os.getenv("UPLOAD_DAILY_LIMIT", "8")))
UPLOAD_INTERVAL_MINUTES = max(0, int(os.getenv("UPLOAD_INTERVAL_MINUTES", "180")))
UPLOAD_WORKER_POLL_SECONDS = max(15, int(os.getenv("UPLOAD_WORKER_POLL_SECONDS", "30")))
UPLOAD_STALE_MINUTES = max(10, int(os.getenv("UPLOAD_STALE_MINUTES", "60")))
DEFAULT_HASHTAGS = os.getenv("DEFAULT_HASHTAGS", "#Shorts #YouTubeShorts").strip()
...
class DiscoverRequest(BaseModel):
    ...
    limit: int = Field(default=MAX_DISCOVER, ge=1, le=MAX_DISCOVER)


class QueueAddRequest(BaseModel):
    video_ids: list[str] = Field(default_factory=list, min_length=1, max_length=10000)
...
def normalize_hashtags(value: str | list[str] | None) -> list[str]:
    raw = re.split(r"[\s,]+", value.strip()) if isinstance(value, str) else (value or [])
    result = []
    seen = set()
    for item in raw:
...
def append_hashtags_to_description(description: str, extra_hashtags: str | list[str] | None = None) -> str:
    base = (description or "").strip()
    names = extract_hashtags(base)
    seen = {x.lower() for x in names}
    for name in normalize_hashtags(DEFAULT_HASHTAGS) + normalize_hashtags(extra_hashtags):
        if name.lower() not in seen:
            names.append(name)
            seen.add(name.lower())
    if not names:
        return base[:5000]
    hashtag_line = " ".join("#" + x for x in names)
    if base:
        if hashtag_line.lower() not in base.lower():
            base += "\n\n" + hashtag_line
    else:
        base = hashtag_line
    return base[:5000]
        ...
        try:
            ...
            if entries:
                ...
                return {
                    ...
                    "offset": req.offset,
                    "limit": req.limit,
                    "has_more": len(entries) == req.limit and (req.offset + len(entries)) < MAX_DISCOVER,
                }
...
def youtube_upload(
    ...
    final_description = append_hashtags_to_description(
        source_description.strip() or description.strip() or "Archived Short",
        tags,
    )
...
def youtube_upload_by_id(req: UploadRequest):
    ...
    description = append_hashtags_to_description(
        source_description.strip() or req.description.strip() or "Archived Short",
        req.tags,
    )
...
# ============================================================
# PERSISTENT AUTOMATIC UPLOAD QUEUE
# ============================================================
...
def _recover_stale_uploads(items: list[dict]) -> bool:
    changed = False
    cutoff = _queue_now() - UPLOAD_STALE_MINUTES * 60
    for item in items:
        if item.get("status") == "uploading" and float(item.get("started_at") or 0) < cutoff:
            item["status"] = "queued"
            item["next_attempt_at"] = _queue_now()
            item["error"] = "Recovered after an interrupted upload."
            changed = True
    return changed


def _recent_upload_count(items: list[dict], now: float | None = None) -> int:
    now = now or _queue_now()
    cutoff = now - 24 * 60 * 60
    return sum(
        1 for item in items
        if item.get("status") == "uploaded"
        and float(item.get("uploaded_at") or 0) >= cutoff
    )


def _last_upload_time(items: list[dict]) -> float | None:
    values = [
        float(item.get("uploaded_at"))
        for item in items
        if item.get("status") == "uploaded" and item.get("uploaded_at")
    ]
    return max(values) if values else None


def _queue_status_payload() -> dict:
    items = _read_queue()
    changed = _recover_stale_uploads(items)
    if changed:
        _write_queue(items)
    now = _queue_now()
    recent = _recent_upload_count(items, now)
    last = _last_upload_time(items)
    return {
        "enabled": True,
        "daily_limit": UPLOAD_DAILY_LIMIT,
        "uploaded_last_24h": recent,
        "remaining_last_24h": max(0, UPLOAD_DAILY_LIMIT - recent),
        "interval_minutes": UPLOAD_INTERVAL_MINUTES,
        "last_upload_at": last,
        "queue_total": len(items),
        "queued": sum(1 for x in items if x.get("status") == "queued"),
        "uploading": sum(1 for x in items if x.get("status") == "uploading"),
        "uploaded": sum(1 for x in items if x.get("status") == "uploaded"),
        "failed": sum(1 for x in items if x.get("status") == "failed"),
    }


def _queue_one_item() -> bool:
    if not _worker_lock.acquire(blocking=False):
        return False
    try:
        items = _read_queue()
        if _recover_stale_uploads(items):
            _write_queue(items)
        now = _queue_now()
        if _recent_upload_count(items, now) >= UPLOAD_DAILY_LIMIT:
            return False
        last = _last_upload_time(items)
        if last is not None and now < last + UPLOAD_INTERVAL_MINUTES * 60:
            return False
        ...
        try:
            ...
            result = youtube_upload_by_id(req)
            uploaded_at = _queue_now()
            latest = _read_queue()
            for item in latest:
                if item.get("video_id") == video_id and item.get("status") == "uploading":
                    item.update({
                        "status": "uploaded",
                    ...
                    break
            _write_queue(latest)
            logger.info("Automatic upload succeeded: %s", video_id)
            return True
        except Exception as exc:
            logger.exception("Automatic upload failed: %s", video_id)
            latest = _read_queue()
            for item in latest:
                if item.get("video_id") == video_id and item.get("status") == "uploading":
                    attempts = int(item.get("attempts") or 1)
                    delay = min(6 * 60 * 60, 5 * 60 * (2 ** min(attempts - 1, 5)))
                    item["status"] = "queued" if attempts < 6 else "failed"
                    item["next_attempt_at"] = _queue_now() + delay
...
async def _upload_scheduler_loop():
    logger.info("Automatic upload scheduler started")
    while True:
        try:
            await asyncio.to_thread(_queue_one_item)
        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("Upload scheduler loop error")
        await asyncio.sleep(UPLOAD_WORKER_POLL_SECONDS)


@app.on_event("startup")
async def start_upload_scheduler():
    global _scheduler_task
    if _scheduler_task is None:
        _scheduler_task = asyncio.create_task(_upload_scheduler_loop())


@app.on_event("shutdown")
async def stop_upload_scheduler():
    global _scheduler_task
    if _scheduler_task:
...
def get_upload_queue():
    ...
    if _recover_stale_uploads(items):
        _write_queue(items)
    return {"status": _queue_status_payload(), "items": items[:1000]}


@app.post("/upload-queue/add")
def add_upload_queue(req: QueueAddRequest):
    valid_ids = [x for x in req.video_ids if re.fullmatch(r"[A-Za-z0-9_-]{11}", x)]
    if not valid_ids:
        raise HTTPException(status_code=400, detail="No valid YouTube video IDs were supplied.")
    with _queue_lock:
        items = _read_queue()
        existing = {x.get("video_id") for x in items}
        now = _queue_now()
        added = 0
    ...
    return {
        "ok": True,
        "added": added,
        "skipped_duplicates": len(valid_ids) - added,
        "status": _queue_status_payload(),
    }


@app.post("/upload-queue/run")
def run_upload_queue_now():
    started = _queue_one_item()
    return {"ok": True, "started": started, "status": _queue_status_payload()}


@app.post("/upload-queue/clear")
def clear_upload_queue(req: QueueClearRequest):
    with _queue_lock:
        items = _read_queue()
        removable = {"queued"}
        if req.include_failed:
            removable.add("failed")
        remaining = [x for x in items if x.get("status") not in removable]
        removed = len(items) - len(remaining)
        _write_queue(remaining)
    return {"ok": True, "removed": removed, "status": _queue_status_payload()}
