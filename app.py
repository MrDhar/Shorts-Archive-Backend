...
# Optional proxy for outbound yt-dlp requests. Set this to a proxy URL, e.g.
# "http://user:pass@host:port" (works with residential/rotating proxy
# providers). Render's shared IPs get rate-limited (HTTP 429/403) by YouTube
# far more than a clean residential IP would -- this is left unset by
# default so nothing changes unless you configure it.
...
def js_runtime_args() -> list[str]:
    ...
    # yt-dlp needs "EJS" JS-challenge-solver scripts to run inside whatever
    # runtime we detect below. Those normally ship bundled with the
    # yt-dlp-ejs package (pulled in via yt-dlp[default]), but if that's ever
    # missing/stale (e.g. a cached Docker layer), yt-dlp has no fallback by
    # default and silently fails every extraction. This lets it fetch the
    # scripts from GitHub at runtime as a safety net either way.
...
def run_ytdlp(
    ...
    # Render's shared datacenter IPs get rate-limited/blocked by YouTube
    # (HTTP 429/403 at the network level, before any player-response
    # parsing even happens) far more aggressively than a residential IP
    # would. Route through a proxy if one is configured to work around this.
    if YTDLP_PROXY:
        command.extend([
            "--proxy",
            YTDLP_PROXY,
...
def fetch_source_metadata(
    ...
    """Return the source video's real title and description.

    Prefer the authenticated YouTube Data API for metadata because it is
    independent of yt-dlp's YouTube player-client/PO-token extraction path.
    If the API lookup is unavailable, retain the existing yt-dlp client
    fallback so the downloader remains resilient.
    """

    # Primary path: YouTube Data API.
    # `videos.list(part=snippet)` returns title, description and tags for the
    # requested public video ID and avoids the player-client extraction path.
    try:
        ...
        if items:
            ...
            if title:
                logger.info(
                    "Source metadata loaded via YouTube Data API: %s",
                    video_id,
                )
                return title, description
    except Exception as exc:
        logger.warning(
            "YouTube Data API metadata lookup failed for %s: %s; trying yt-dlp fallback",
            video_id,
            exc,
        )

    # Fallback path: keep the existing yt-dlp metadata extraction.
    video_url = (
        "https://www.youtube.com/shorts/"
        + video_id
    )
    ...
    for client in DISCOVERY_CLIENTS:
        ...
        if title:
            logger.info(
                "Source metadata loaded via yt-dlp fallback (%s): %s",
                client,
                video_id,
            )
            return title, description

    logger.warning("No source metadata available for %s", video_id)
    return "", ""
...
@app.options("/health")
def health_options():
    # Some monitoring tools / network layers send an OPTIONS probe before
    # the real check. Without this, FastAPI 405s on it since only GET/HEAD
    # are registered above.
    return {"ok": True}


# ============================================================
# TEMPORARY DIAGNOSTIC -- remove once the extraction issue is solved.
# No shell access on Render's free tier, so this surfaces yt-dlp's raw
# version + a real invocation's raw stdout/stderr directly over HTTP.
# ============================================================

@app.get("/debug/ytdlp", response_class=PlainTextResponse)
def debug_ytdlp(url: str = "https://www.youtube.com/shorts/xpGNUtkZams"):
    out = []
    ...
    out.append("=== yt-dlp --version ===")
    out.append(version.stdout.strip())
    out.append(version.stderr.strip())
...
def upload_queue_run(request: Request):
    ...
    # Cron services (GitHub Actions, cron-job.org, etc.) commonly time out
    # requests around 30s, but a real upload run can take many minutes.
    # So: return immediately and do the actual work on a background thread.
    # The lock above is released inside the thread once it finishes, which
    # is exactly what makes the "already running" check above meaningful
    # for the *next* cron ping that comes in while this one is still going.
    thread = threading.Thread(target=_run_upload_queue_worker, daemon=True)
    thread.start()

    return {
        "ok": True,
        "started": True,
        "reason": "processing in background; poll GET /upload-queue for status",
    }


def _run_upload_queue_worker() -> None:
    try:
        db = _supabase_required()
        data = _load_queue()
        now = time.time()
        ...
        if cooldown_until > now:
            logger.info(
                "Queue worker skipped: waiting %ds for YouTube daily limit cooldown (resets %s)",
                int(cooldown_until - now),
                time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(cooldown_until)),
            )
            return
        ...
        for item in queued_items:
            ...
            try:
                ...
                if daily_limit:
                    # This is YouTube's per-channel upload safety limit (not
                    # the API project quota, which is a different thing that
                    # resets at a fixed midnight-PT boundary). Google's own
                    # docs are explicit that this one is a rolling 24h window
                    # measured from when you hit it, not a fixed daily reset:
                    # "we limit how many videos a channel can upload in a
                    # 24-hour period... try again in 24 hours." So a flat
                    # +24h from right now is the correct model here.
        ...
        logger.exception("Queue worker crashed unexpectedly")
